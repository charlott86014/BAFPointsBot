...
exports.run = async (client, message, args) => {
    ...
    if(Number(process.env.BRGMaximumRank) <= rankInGroup || Number(process.env.BRGMaximumRank) <= rankInGroup + 1){
        return message.channel.send({embeds: [{
            color: 16711680,
    ...
    let newRank = await getRankID(Number(process.env.ArmyBRGGroupId), id);
