        ...
        default:
            if (typeof pointsToRemove === typeof undefined) {
                ...
                dbconnection.query('SELECT * FROM `UserPoints` WHERE `id_user` = '+user+'' , (err, rows) => {
                    ...
                    if (brgnewpointsamount < 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                            description: `Points cannot be negative!`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    } else if (brgnewpointsamount < 0 && rows[0].brgpoints > 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                        ...
                        message.channel.send({embeds: [{
                            color: 65280,
                            description: `${user} has ${pointsToRemove} points less, and now stands at ${brgnewpointsamount} points.`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    }
                    const member = message.mentions.members.first();

                    if (brgnewpointsamount < 10 && member.roles.cache.some(role => role.name === 'Royal Guard') || brgnewpointsamount < 30 && member.roles.cache.some(role => role.name === 'Professional Guard') ||
                        brgnewpointsamount < 50 && member.roles.cache.some(role => role.name === 'Expert Guard')) {
