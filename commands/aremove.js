        ...
        default:
            if (typeof pointsToRemove === typeof undefined) {
                ...
                dbconnection.query('SELECT * FROM `UserPoints` WHERE `id_user` = '+user+'' , (err, rows) => {
                    ...
                    if (anewpointsamount < 0 && rows[0].apoints < 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                            description: `Points cannot be negative!`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    } else if (anewpointsamount < 0 && rows[0].apoints > 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                        ...
                        message.channel.send({embeds: [{
                            color: 65280,
                            description: `${user} has ${pointsToRemove} points less, and now stands at ${anewpointsamount} points.`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    }
                    const member = message.mentions.members.first();

                    if (anewpointsamount < 5 && member.roles.cache.some(role => role.name === 'Private [LR]') || anewpointsamount < 10 && member.roles.cache.some(role => role.name === 'Private First Class [LR]') ||
                        anewpointsamount < 15 && member.roles.cache.some(role => role.name === 'Corporal [LR]') || anewpointsamount < 20 && member.roles.cache.some(role => role.name === 'Chief Corporal [MR]') || 
                        anewpointsamount < 30 && member.roles.cache.some(role => role.name === 'First Chief Corporal [MR]') || anewpointsamount < 40 && member.roles.cache.some(role => role.name === 'Sergeant [MR]') || 
                        anewpointsamount < 55 && member.roles.cache.some(role => role.name === 'Chief Sergeant [MR]') || anewpointsamount < 70 && member.roles.cache.some(role => role.name === 'First Master Sergeant [MR]') ||
                        anewpointsamount < 90 && member.roles.cache.some(role => role.name === 'First Chief Sergeant [MR]')) {
                        demotion = async (message) => {
                            ...
                            try {
                                demoteResponse = await roblox.demote(Number(process.env.ArmyGroupId), id);
                            } catch (err) {
                                console.log(chalk.red('An error occured while demoting: ' + err));
                                return message.channel.send({embeds: [{
                                    color: 16711680,
