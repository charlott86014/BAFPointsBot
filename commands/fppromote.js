...
exports.run = async (client, message, args) => {
    ...
    if(Number(process.env.FPMaximumRank) <= rankInGroup || Number(process.env.FPMaximumRank) <= rankInGroup + 1){
        return message.channel.send({embeds: [{
            color: 16711680,
    ...
    let newRank = await getRankID(Number(process.env.ArmyFPGroupId), id);
