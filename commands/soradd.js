        ...
        default:
            if (typeof pointsToAdd === typeof undefined) {
                ...
                dbconnection.query('SELECT * FROM `UserPoints` WHERE `id_user` = '+user+'' , (err, rows) => {
                    ...
                    if (sornewpointsamount >= 35 && !(rows[0].sorpoints < 35)) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                            description: `${user} has already got the maximum points for this divison!`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    } else if (sornewpointsamount >= 35 && rows[0].sorpoints < 35){
                        ...
                        message.channel.send({embeds: [{
                            color: 65280,
                            description: `${user} has received ${pointsToAdd} points, and now stands at ${sornewpointsamount} points.`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    }
                    const member = message.mentions.members.first();

                    if (sornewpointsamount > 9 && member.roles.cache.some(role => role.name === 'Junior Operator') || sornewpointsamount > 19 && member.roles.cache.some(role => role.name === 'Operator') || 
                        sornewpointsamount > 34 && member.roles.cache.some(role => role.name === 'Senior Operator')) {
                        promotion = async (message) => {
                            ...
                            try {
                                promoteResponse = await roblox.promote(Number(process.env.SORGroupId), id);
                            } catch (err) {
                                console.log(chalk.red('An error occured while promoting: ' + err));
                                return message.channel.send({embeds: [{
                                    color: 16711680,
