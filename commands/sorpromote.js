...
exports.run = async (client, message, args) => {
    ...
    if(Number(process.env.SORMaximumRank) <= rankInGroup || Number(process.env.SORMaximumRank) <= rankInGroup + 1){
        return message.channel.send({embeds: [{
            color: 16711680,
    ...
    let newRankName = await getRankName(Number(process.env.SORGroupId), id);
    let newRank = await getRankID(Number(process.env.ArmySORGroupId), id);
