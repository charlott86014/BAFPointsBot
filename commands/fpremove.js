        ...
        default:
            if (typeof pointsToRemove === typeof undefined) {
                ...
                dbconnection.query('SELECT * FROM `UserPoints` WHERE `id_user` = '+user+'' , (err, rows) => {
                    ...
                    if (fpnewpointsamount < 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                            description: `Points cannot be negative!`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    } else if (fpnewpointsamount < 0 && rows[0].fppoints > 0) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                        ...
                        message.channel.send({embeds: [{
                            color: 65280,
                            description: `${user} has ${pointsToRemove} points less, and now stands at ${fpnewpointsamount} points.`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    }
                    const member = message.mentions.members.first();

                    if (fpnewpointsamount < 5 && member.roles.cache.some(role => role.name === 'Agent') ||fpnewpointsamount < 10 && member.roles.cache.some(role => role.name === 'Vice Inspector') || 
                    fpnewpointsamount < 15 && member.roles.cache.some(role => role.name === 'Inspector') || fpnewpointsamount < 20 && member.roles.cache.some(role => role.name === 'Vice Chief Inspector') || 
                    fpnewpointsamount < 25 && member.roles.cache.some(role => role.name === 'Chief Inspector') || fpnewpointsamount < 35 && member.roles.cache.some(role => role.name === 'Vice Inspector')) {
                        demotion = async (message) => {
                            ...
                            try {
                                demoteResponse = await roblox.demote(Number(process.env.FPGroupId), id);
                            } catch (err) {
                                console.log(chalk.red('An error occured while demoting: ' + err));
                                return message.channel.send({embeds: [{
                                    color: 16711680,
