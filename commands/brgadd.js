...
exports.run = async (client, message, args) => {
    if (message.guild.id !== process.env.BRGGuildId) {
        ...
        default:
            if (typeof pointsToAdd === typeof undefined) {
                ...
                dbconnection.query('SELECT * FROM `UserPoints` WHERE `id_user` = '+user+'' , (err, rows) => {
                    ...
                    if (brgnewpointsamount >= 35 && !(rows[0].brgpoints < 35)) {
                        message.channel.send({embeds: [{
                            color: 16737095,
                            description: `${user} has already got the maximum points for this divison!`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    } else if (brgnewpointsamount >= 35 && rows[0].brgpoints < 35){
                        sql = 'UPDATE `UserPoints` SET `brgpoints` = '+35+' WHERE `id_user` = '+user+'';
                        ...
                        sql = 'UPDATE `UserPoints` SET `brgpoints` = '+brgnewpointsamount+' WHERE `id_user` = '+user+''
                        dbconnection.query(sql);
                        message.channel.send({embeds: [{
                            color: 65280,
                            description: `${user} has received ${pointsToAdd} points, and now stands at ${brgnewpointsamount} points.`,
                            author: {name: message.author.tag, icon_url: message.author.displayAvatarURL()}
                        }]});
                    }
                    const member = message.mentions.members.first();

                    if (brgnewpointsamount > 9 && member.roles.cache.some(role => role.name === 'Enlistment') || brgnewpointsamount > 29 && member.roles.cache.some(role => role.name === 'Royal Guard') || 
                        brgnewpointsamount > 49 && member.roles.cache.some(role => role.name === 'Professional Guard')) {
                        promotion = async (message) => {
                            ...
                            try {
                                promoteResponse = await roblox.promote(Number(process.env.BRGGroupId), id);
                            } catch (err) {
                                console.log(chalk.red('An error occured while promoting: ' + err));
                                return message.channel.send({embeds: [{
                                    color: 16711680,
                            ...
                            if(process.env.BRGLogChannelId === 'false') return;
                            let logchannel = await message.guild.channels.cache.get(process.env.BRGLogChannelId);
